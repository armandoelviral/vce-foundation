...
from tests.test_security_admission_candidate_indeterminate_byte_length_comparison_result_binding import (
    create_binding as create_candidate_byte_binding,
)
...
def create_binding(
    ...
    candidate_byte = create_candidate_byte_binding()
    ...
    record = replace(
        ...
        evaluation_identity=replace(
            ...
            evaluation_basis=candidate_byte.evaluation_basis,
        ),
    )
    requirements = replace(
        initial_requirements_binding.policy_evidence_requirements,
        admission_policy_identity=(
            candidate_byte.evaluation_basis.admission_policy_identity
        ),
    )
    ...
    recorded_byte = (
        SecurityAdmissionEvaluationRecordIndeterminateByteLengthComparisonResultBinding(
            evaluation_record=record,
            indeterminate_byte_length_comparison_result_binding=candidate_byte,
        )
    )
    return (
        SecurityAdmissionEvidenceCoverageIndeterminateByteLengthComparisonResultBinding(
...
def test_indeterminate_result_participates_in_binding_value() -> None:
    ...
    candidate_binding = (
        recorded.indeterminate_byte_length_comparison_result_binding
    )
    result = candidate_binding.indeterminate_byte_length_comparison_result
