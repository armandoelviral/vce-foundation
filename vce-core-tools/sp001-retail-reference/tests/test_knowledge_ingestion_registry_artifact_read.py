...
def test_read_status_vocabulary_is_exact() -> None:
    assert tuple(
        status.value
        for status in KnowledgeIngestionRegistryReadStatus
    ) == (
        "LOADED",
        "NOT_FOUND",
    )
...
def test_read_result_is_immutable(
    ...
    result = KnowledgeIngestionRegistryReadResult(
        status=KnowledgeIngestionRegistryReadStatus.NOT_FOUND,
    ...
    with pytest.raises(
        ...
        result.status = KnowledgeIngestionRegistryReadStatus.LOADED
...
def test_read_result_rejects_untyped_status(
    ...
    with pytest.raises(
        TypeError,
        match="KnowledgeIngestionRegistryReadStatus",
    ):
        KnowledgeIngestionRegistryReadResult(
            status="NOT_FOUND",
            artifact_path=create_location(tmp_path).artifact_path,
...
def test_reader_rejects_untyped_location(
    ...
    with pytest.raises(
        TypeError,
        match="KnowledgeIngestionRegistryStorageLocation",
    ):
        read_knowledge_ingestion_registry_artifact(
            location=location,
        )
...
def test_missing_artifact_returns_not_found(
    ...
    result = read_knowledge_ingestion_registry_artifact(
        location=location,
    )
...
def test_written_artifact_is_loaded(
    ...
    assert result.status is (
        KnowledgeIngestionRegistryReadStatus.LOADED
    )
    assert result.artifact_path == location.artifact_path
...
def test_loaded_artifact_preserves_unicode(
    ...
    assert result.artifact == artifact
    assert "INGESTIÓN-Ñ" in result.artifact.payload
