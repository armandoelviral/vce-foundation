...
def candidate_from_result(
    result: SecurityAdmissionCandidateMediaTypeComparisonResult,
):
    return (
        result
        .comparison_basis
...
def create_binding(
    ...
    return SecurityAdmissionCandidateMediaTypeComparisonResultBinding(
...
def test_binding_fields_are_exact() -> None:
    ...
    assert (
        ...
        is SecurityAdmissionCandidateMediaTypeComparisonResult
    )
...
def test_evaluation_basis_requires_exact_type(invalid_value) -> None:
    with pytest.raises(
        ...
        SecurityAdmissionCandidateMediaTypeComparisonResultBinding(
            ...
            media_type_comparison_result=create_result(),
        )


@pytest.mark.parametrize("invalid_value", (None, 1, True, object()))
def test_media_type_result_requires_exact_type(invalid_value) -> None:
    with pytest.raises(
        TypeError,
        match=(
        ...
        SecurityAdmissionCandidateMediaTypeComparisonResultBinding(
            ...
            media_type_comparison_result=invalid_value,
        )
...
def test_exact_evaluation_basis_reference_is_preserved() -> None:
    result = create_result()
    evaluation_basis = evaluation_basis_for_result(result)
    binding = SecurityAdmissionCandidateMediaTypeComparisonResultBinding(
...
def test_exact_result_reference_is_preserved() -> None:
    ...
    assert binding.media_type_comparison_result is result
    ...
    reconstructed = (
        SecurityAdmissionCandidateMediaTypeComparisonResultBinding(
            ...
            media_type_comparison_result=replace(
                binding.media_type_comparison_result
            ),
        )
    )
...
@pytest.mark.parametrize(
    "status",
    tuple(SecurityAdmissionMediaTypeComparisonStatus),
)
def test_conclusive_status_is_preserved_without_interpretation(
    ...
    assert binding.media_type_comparison_result.match_status is status
...
def test_indeterminate_result_cannot_occupy_conclusive_slot() -> None:
    with pytest.raises(TypeError):
        SecurityAdmissionCandidateMediaTypeComparisonResultBinding(
            evaluation_basis=create_evaluation_basis(),
            media_type_comparison_result=create_indeterminate_result(),
        )
