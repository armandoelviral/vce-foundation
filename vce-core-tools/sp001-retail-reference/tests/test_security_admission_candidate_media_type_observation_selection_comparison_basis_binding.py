...
def create_binding(
    ...
    comparison_basis = replace(
        ...
        detected_media_type_observation=selection.selected_observation,
    )
    return (
        SecurityAdmissionCandidateMediaTypeObservationSelectionComparisonBasisBinding(
...
def test_fields_are_exact() -> None:
    ...
    assert (
        ...
        is SecurityAdmissionCandidateMediaTypeObservationSelectionResult
    )
    assert (
        binding_fields[1].type
        is SecurityAdmissionCandidateMediaTypeComparisonBasis
    )
...
def test_exact_references_are_preserved() -> None:
    ...
    assert (
        reconstructed.comparison_basis.detected_media_type_observation
...
def test_equal_reconstructed_selected_observation_is_permitted() -> None:
    ...
    comparison_basis = replace(
        ...
        detected_media_type_observation=reconstructed_selected,
    )
    preserved = replace(
        binding,
        comparison_basis=comparison_basis,
    ...
    assert (
        preserved.comparison_basis.detected_media_type_observation
...
def test_different_observation_version_is_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="selected media-type observation"):
        replace(binding, comparison_basis=comparison_basis)
