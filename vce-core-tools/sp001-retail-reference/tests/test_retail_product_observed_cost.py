...
def create_binding(
    *,
    ...
    dimension = RetailContextDimension(
        dimension_id="cost-observation-001",
        dimension_type="PRODUCT_COST",
    ...
    provenance = RetailContextObservationProvenance(
        observation_id="cost-observation-001",
        ...
        evidence_ids=("cost-evidence-001",),
    )
...
def test_observed_cost_fields_are_exact() -> None:
    assert tuple(
        field.name
        for field in fields(RetailProductObservedCost)
    ) == (
        "product_identity",
        "context_scope",
        "monetary_amount",
        "costing_scheme_id",
        "costing_scheme_version",
        "provenance_binding",
    )


def test_observed_cost_is_immutable() -> None:
    observation = create_observed_cost()

    with pytest.raises(FrozenInstanceError):
        observation.costing_scheme_version = 2  # type: ignore[misc]


def test_observed_cost_uses_slots() -> None:
    assert not hasattr(create_observed_cost(), "__dict__")
...
def test_exact_typed_references_are_preserved() -> None:
    ...
    observation = RetailProductObservedCost(
...
def test_typed_members_require_exact_types(
    ...
    with pytest.raises(TypeError, match=message):
        replace(
            create_observed_cost(),
            **{field_name: object()},
        )


@pytest.mark.parametrize("minor_unit_amount", [0, 1, 12500, 10**30])
def test_nonnegative_cost_amounts_are_accepted_exactly(
...
def test_negative_cost_amounts_are_rejected(
    minor_unit_amount: int,
) -> None:
    with pytest.raises(
        ValueError,
...
def test_generic_signed_amount_remains_valid_outside_cost() -> None:
...
def test_costing_scheme_id_requires_string(value: object) -> None:
    with pytest.raises(
        TypeError,
        match="costing_scheme_id must be a string",
    ):
        replace(
            create_observed_cost(),
            costing_scheme_id=value,  # type: ignore[arg-type]
        )


def test_empty_costing_scheme_id_is_rejected() -> None:
    with pytest.raises(
        ValueError,
        match="costing_scheme_id must not be empty",
    ):
        replace(
            create_observed_cost(),
            costing_scheme_id="",
...
@pytest.mark.parametrize("value", [" ", "\t", " cost scheme "])
def test_costing_scheme_id_is_preserved_without_normalization(
    value: str,
) -> None:
    observation = replace(
        create_observed_cost(),
        costing_scheme_id=value,
    )

    assert observation.costing_scheme_id == value


@pytest.mark.parametrize("value", [True, False, 1.0, "1", None])
def test_costing_scheme_version_requires_strict_integer(
    value: object,
) -> None:
    with pytest.raises(
        TypeError,
        match="costing_scheme_version must be an integer",
    ):
        replace(
            create_observed_cost(),
            costing_scheme_version=value,  # type: ignore[arg-type]
        )


@pytest.mark.parametrize("value", [0, -1, -(10**30)])
def test_costing_scheme_version_requires_positive_value(
    value: int,
) -> None:
    with pytest.raises(
        ValueError,
        match="costing_scheme_version must be positive",
    ):
        replace(
            create_observed_cost(),
            costing_scheme_version=value,
        )
...
def test_cost_observation_has_no_aggregation_period() -> None:
    observation = create_observed_cost()
...
def test_zero_cost_remains_an_uninterpreted_observation() -> None:
    observation = create_observed_cost(minor_unit_amount=0)

    assert observation.monetary_amount.minor_unit_amount == 0
    assert not hasattr(observation, "cost_status")
    assert not hasattr(observation, "availability")
    assert not hasattr(observation, "commercial_decision")


def test_contract_adds_no_cost_basis_or_margin_fields() -> None:
    field_names = {
        field.name
        for field in fields(RetailProductObservedCost)
    }
...
def test_contract_imports_no_external_or_execution_capability() -> None:
    module = inspect.getmodule(RetailProductObservedCost)
