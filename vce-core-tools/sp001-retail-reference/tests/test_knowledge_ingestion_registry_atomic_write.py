...
def test_atomic_write_returns_immutable_result(
    ...
    result = write_knowledge_ingestion_registry_artifact(
        location=create_location(
            tmp_path,
        ),
        artifact=create_artifact(),
...
def test_atomic_write_replaces_existing_artifact(
    ...
    original = create_artifact(
        ingestion_id="INGESTION-ORIGINAL",
    )

    replacement = create_artifact(
        ingestion_id="INGESTION-REPLACEMENT",
    )
...
def test_replace_receives_sibling_temporary_path(
    ...
    monkeypatch.setattr(
        "sp001.services."
        "knowledge_ingestion_registry_atomic_write."
        "os.replace",
        observing_replace,
    )
...
def test_atomic_write_requires_no_registry_reconstruction(
    tmp_path: Path,
) -> None:
    artifact = create_artifact()
