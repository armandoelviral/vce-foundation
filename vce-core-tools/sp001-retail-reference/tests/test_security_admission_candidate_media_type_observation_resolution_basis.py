...
def create_basis(
    ...
    return SecurityAdmissionCandidateMediaTypeObservationResolutionBasis(
...
def test_fields_are_exact() -> None:
    ...
    assert (
        basis_fields[0].type
        is SecurityAdmissionCandidateMediaTypeObservationSet
    )
    assert (
        basis_fields[1].type
...
def test_exact_references_are_preserved() -> None:
    ...
    basis = SecurityAdmissionCandidateMediaTypeObservationResolutionBasis(
        observation_set=observation_set,
        authority_order=authority_order,
    )
...
def test_non_media_type_authority_order_is_rejected() -> None:
    basis = create_basis()
