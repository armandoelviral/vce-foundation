...
def create_set() -> SecurityAdmissionCandidateByteLengthObservationSet:
    first = create_observation()
    second = replace(first, observation_id="observation-002")
    return SecurityAdmissionCandidateByteLengthObservationSet(
...
def test_observation_value_participates_in_set_value() -> None:
    collection = create_set()
    changed = replace(
        collection.observations[0], measured_byte_length=collection.observations[0].measured_byte_length + 1
    )
    assert replace(
        collection,
...
def test_no_coverage_or_admission_fields() -> None:
    names = {
        field.name
        for field in fields(SecurityAdmissionCandidateByteLengthObservationSet)
    }
