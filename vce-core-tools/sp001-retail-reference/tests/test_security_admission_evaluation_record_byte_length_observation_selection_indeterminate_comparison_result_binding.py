...
from tests.test_security_admission_candidate_byte_length_observation_selection_indeterminate_comparison_result_binding import (
    create_binding as create_governed_binding,
)
from tests.test_security_admission_evaluation_record_indeterminate_byte_length_comparison_result_binding import (
...
def create_binding(
    ...
    recorded = create_recorded_binding(
        indeterminate_byte_length_comparison_result_binding=(
            governed.indeterminate_byte_length_comparison_result_binding
        ),
    )
    return (
        SecurityAdmissionEvaluationRecordByteLengthObservationSelectionIndeterminateComparisonResultBinding(
...
def test_fields_are_exact() -> None:
    ...
    assert (
        ...
        is SecurityAdmissionCandidateByteLengthObservationSelectionIndeterminateComparisonResultBinding
    )
    assert (
        binding_fields[1].type
        is SecurityAdmissionEvaluationRecordIndeterminateByteLengthComparisonResultBinding
    )
...
def test_binding_is_immutable_and_slotted() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        binding.evaluation_record_indeterminate_byte_length_comparison_result_binding = (  # type: ignore[misc]
            create_recorded_binding()
        )
...
def test_exact_references_are_preserved() -> None:
    ...
    recorded = (
        binding.evaluation_record_indeterminate_byte_length_comparison_result_binding
    )
    reconstructed = (
        SecurityAdmissionEvaluationRecordByteLengthObservationSelectionIndeterminateComparisonResultBinding(
            observation_selection_indeterminate_comparison_result_binding=governed,
    ...
    assert (
        reconstructed.evaluation_record_indeterminate_byte_length_comparison_result_binding
        is recorded
    )
...
def test_complete_selection_and_evaluation_lineage_remains_reachable() -> None:
    ...
    assert (
        ...
        == governed.indeterminate_byte_length_comparison_result_binding.evaluation_basis
    )
...
def test_equal_reconstruction_has_value_equality() -> None:
    ...
    reconstructed = replace(
        ...
        evaluation_record_indeterminate_byte_length_comparison_result_binding=replace(
            binding.evaluation_record_indeterminate_byte_length_comparison_result_binding
        ),
    )
...
def test_different_indeterminate_candidate_result_binding_is_rejected() -> None:
    ...
    changed_result = replace(
        candidate_result_binding.indeterminate_byte_length_comparison_result,
        result_version=(
            candidate_result_binding
