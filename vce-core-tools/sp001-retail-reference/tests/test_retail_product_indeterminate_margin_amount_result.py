...
from sp001.contracts.retail_product_indeterminate_margin_amount_result import (
    RetailProductIndeterminateMarginAmountResult,
    RetailProductMarginAmountIndeterminacyReason,
)
...
def create_result(
    reason: RetailProductMarginAmountIndeterminacyReason = (
        RetailProductMarginAmountIndeterminacyReason
        .INSUFFICIENT_INPUT_EVIDENCE
    ),
    determined_at: datetime = DETERMINED_AT,
) -> RetailProductIndeterminateMarginAmountResult:
    return RetailProductIndeterminateMarginAmountResult(
        result_id="indeterminate-margin-amount-001",
        result_version=1,
...
def test_reason_vocabulary_is_exact() -> None:
    assert tuple(reason.value for reason in RetailProductMarginAmountIndeterminacyReason) == (
...
def test_result_fields_are_exact() -> None:
    assert tuple(field.name for field in fields(RetailProductIndeterminateMarginAmountResult)) == (
        "result_id",
        "result_version",
        "calculation_basis",
        "reason",
...
def test_result_is_frozen_and_slotted() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        result.reason = RetailProductMarginAmountIndeterminacyReason.UNSUPPORTED_CALCULATION_SCHEME  # type: ignore[misc]
...
@pytest.mark.parametrize("reason", tuple(RetailProductMarginAmountIndeterminacyReason))
def test_each_typed_reason_is_accepted(reason: RetailProductMarginAmountIndeterminacyReason) -> None:
...
def test_reason_requires_exact_enum_type(value: object) -> None:
    with pytest.raises(TypeError, match="reason must be a RetailProductMarginAmountIndeterminacyReason"):
        replace(create_result(), reason=value)  # type: ignore[arg-type]


def test_denominator_reason_is_not_available() -> None:
    assert not hasattr(RetailProductMarginAmountIndeterminacyReason, "NONPOSITIVE_DENOMINATOR")
...
def test_successful_result_fields_are_absent() -> None:
    names = {field.name for field in fields(RetailProductIndeterminateMarginAmountResult)}
...
def test_contract_performs_no_calculation_or_exception_mapping() -> None:
    module = inspect.getmodule(RetailProductIndeterminateMarginAmountResult)
