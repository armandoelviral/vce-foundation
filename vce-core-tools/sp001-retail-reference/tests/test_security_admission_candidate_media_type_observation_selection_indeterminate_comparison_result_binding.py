...
from tests.test_security_admission_candidate_media_type_observation_selection_comparison_basis_binding import (
...
def create_binding(
    ...
    comparison_result = replace(
        initial_result_binding.indeterminate_media_type_comparison_result,
        comparison_basis=governed.comparison_basis,
    )
    result_binding = replace(
        initial_result_binding,
        indeterminate_media_type_comparison_result=comparison_result,
    )
    return (
        SecurityAdmissionCandidateMediaTypeObservationSelectionIndeterminateComparisonResultBinding(
            observation_selection_comparison_basis_binding=governed,
            indeterminate_media_type_comparison_result_binding=result_binding,
        )
    )
...
def test_fields_are_exact() -> None:
    ...
    assert (
        ...
        is SecurityAdmissionCandidateMediaTypeObservationSelectionComparisonBasisBinding
    )
    assert (
...
def test_binding_is_immutable_and_slotted() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        binding.indeterminate_media_type_comparison_result_binding = (  # type: ignore[misc]
...
def test_exact_references_are_preserved() -> None:
    ...
    result_binding = binding.indeterminate_media_type_comparison_result_binding
    reconstructed = (
        SecurityAdmissionCandidateMediaTypeObservationSelectionIndeterminateComparisonResultBinding(
            observation_selection_comparison_basis_binding=governed,
            indeterminate_media_type_comparison_result_binding=result_binding,
    ...
    assert (
        reconstructed.indeterminate_media_type_comparison_result_binding
        is result_binding
    )
...
def test_complete_selection_lineage_remains_reachable() -> None:
    ...
    assert governed.comparison_basis.detected_media_type_observation == (
...
def test_equal_reconstructed_basis_is_permitted() -> None:
    binding = create_binding()
    result_binding = binding.indeterminate_media_type_comparison_result_binding
    result = result_binding.indeterminate_media_type_comparison_result
    ...
    reconstructed_result_binding = replace(
        result_binding,
        indeterminate_media_type_comparison_result=reconstructed_result,
    )
    preserved = replace(
        binding,
        indeterminate_media_type_comparison_result_binding=(
...
def test_different_comparison_scheme_is_rejected() -> None:
    ...
    different_result_binding = replace(
        result_binding,
        indeterminate_media_type_comparison_result=different_result,
    )
    with pytest.raises(
        ValueError,
        match=(
            "indeterminate media-type comparison result must use the governed "
            "comparison basis"
        ),
    ):
        replace(
...
def test_different_selected_observation_basis_is_rejected() -> None:
    ...
    result_binding = binding.indeterminate_media_type_comparison_result_binding
    result = result_binding.indeterminate_media_type_comparison_result
    selected = result.comparison_basis.detected_media_type_observation
    ...
    different_basis = replace(
        result.comparison_basis,
        detected_media_type_observation=different_observation,
    )
    different_result = replace(
        result,
        comparison_basis=different_basis,
    ...
    with pytest.raises(
        ValueError,
        match="indeterminate media-type comparison result must use the governed",
    ):
        replace(
            binding,
            indeterminate_media_type_comparison_result_binding=(
...
def test_each_binding_participates_in_value() -> None:
    ...
    changed_result = replace(
        result_binding.indeterminate_media_type_comparison_result,
        result_version=(
            result_binding.indeterminate_media_type_comparison_result.result_version + 1
        ),
    )
    ...
    assert replace(
        binding,
        indeterminate_media_type_comparison_result_binding=changed_result_binding,
    ) != binding
