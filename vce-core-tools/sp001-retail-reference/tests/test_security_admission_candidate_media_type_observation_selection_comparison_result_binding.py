...
def create_binding(
    ...
    comparison_result = replace(
        initial_result_binding.media_type_comparison_result,
        comparison_basis=governed.comparison_basis,
    )
    result_binding = replace(
        initial_result_binding,
        media_type_comparison_result=comparison_result,
    )
    return (
        SecurityAdmissionCandidateMediaTypeObservationSelectionComparisonResultBinding(
            observation_selection_comparison_basis_binding=governed,
            media_type_comparison_result_binding=result_binding,
        )
    )
...
def test_binding_is_immutable_and_slotted() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        binding.media_type_comparison_result_binding = (  # type: ignore[misc]
...
def test_exact_references_are_preserved() -> None:
    ...
    result_binding = binding.media_type_comparison_result_binding
    reconstructed = (
        SecurityAdmissionCandidateMediaTypeObservationSelectionComparisonResultBinding(
            observation_selection_comparison_basis_binding=governed,
            media_type_comparison_result_binding=result_binding,
    ...
    assert (
        reconstructed.media_type_comparison_result_binding
        is result_binding
    )
...
def test_complete_conclusive_result_remains_reachable() -> None:
    binding = create_binding()
    result = (
        binding.media_type_comparison_result_binding
        .media_type_comparison_result
...
def test_equal_reconstructed_basis_is_permitted() -> None:
    binding = create_binding()
    result_binding = binding.media_type_comparison_result_binding
    result = result_binding.media_type_comparison_result
    ...
    reconstructed_result_binding = replace(
        result_binding,
        media_type_comparison_result=reconstructed_result,
    )
    preserved = replace(
        binding,
        media_type_comparison_result_binding=(
...
def test_different_comparison_scheme_is_rejected() -> None:
    ...
    different_result_binding = replace(
        result_binding,
        media_type_comparison_result=different_result,
    )
    with pytest.raises(
        ValueError,
        match=(
...
def test_different_selected_observation_basis_is_rejected() -> None:
    ...
    with pytest.raises(
        ValueError,
        match="media-type comparison result must use the governed",
    ):
        replace(
            binding,
            media_type_comparison_result_binding=(
...
def test_each_binding_participates_in_value() -> None:
    ...
    changed_result = replace(
        ...
        result_version=(
            result_binding.media_type_comparison_result.result_version + 1
        ),
    )
