...
from tests.test_security_admission_evaluation_record_byte_length_observation_selection_indeterminate_comparison_result_binding import (
    create_binding as create_governed_recorded_binding,
)
from tests.test_security_admission_evidence_coverage_indeterminate_byte_length_comparison_result_binding import (
...
def create_binding(
    ...
    coverage_binding = (
        SecurityAdmissionEvidenceCoverageIndeterminateByteLengthComparisonResultBinding(
            ...
            evaluation_record_indeterminate_byte_length_comparison_result_binding=recorded,
        )
    )
    return (
        SecurityAdmissionEvidenceCoverageByteLengthObservationSelectionIndeterminateComparisonResultBinding(
...
def test_fields_are_exact() -> None:
    ...
    assert (
        ...
        is SecurityAdmissionEvaluationRecordByteLengthObservationSelectionIndeterminateComparisonResultBinding
    )
    assert (
        binding_fields[1].type
        is SecurityAdmissionEvidenceCoverageIndeterminateByteLengthComparisonResultBinding
    )
...
def test_binding_is_immutable_and_slotted() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        binding.evidence_coverage_indeterminate_byte_length_comparison_result_binding = (  # type: ignore[misc]
...
def test_exact_references_are_preserved() -> None:
    ...
    coverage = (
        binding.evidence_coverage_indeterminate_byte_length_comparison_result_binding
    )
    reconstructed = (
        SecurityAdmissionEvidenceCoverageByteLengthObservationSelectionIndeterminateComparisonResultBinding(
            evaluation_record_byte_length_observation_selection_indeterminate_comparison_result_binding=(
            ...
            evidence_coverage_indeterminate_byte_length_comparison_result_binding=coverage,
        )
    )
    assert (
        reconstructed
...
def test_governed_recorded_indeterminate_result_is_preserved_in_coverage() -> None:
    binding = create_binding()
    governed_recorded = (
        binding
        .evaluation_record_byte_length_observation_selection_indeterminate_comparison_result_binding
...
def test_complete_selection_evaluation_and_coverage_lineage_is_reachable() -> None:
    ...
    selection = (
        governed.observation_selection_indeterminate_comparison_result_binding
        .observation_selection_comparison_basis_binding
        .observation_selection_result
    )
    recorded = (
        governed.evaluation_record_indeterminate_byte_length_comparison_result_binding
    )
    coverage_record = (
...
def test_equal_reconstruction_has_value_equality() -> None:
    ...
    reconstructed = replace(
        binding,
        evaluation_record_byte_length_observation_selection_indeterminate_comparison_result_binding=replace(
            binding
            .evaluation_record_byte_length_observation_selection_indeterminate_comparison_result_binding
        ),
        evidence_coverage_indeterminate_byte_length_comparison_result_binding=replace(
            binding.evidence_coverage_indeterminate_byte_length_comparison_result_binding
        ),
    )
...
def test_different_recorded_indeterminate_result_binding_is_rejected() -> None:
    binding = create_binding()
    coverage = (
        binding.evidence_coverage_indeterminate_byte_length_comparison_result_binding
    )
    recorded = (
        coverage.evaluation_record_indeterminate_byte_length_comparison_result_binding
    )
    candidate_binding = recorded.indeterminate_byte_length_comparison_result_binding
    changed_result = replace(
        candidate_binding.indeterminate_byte_length_comparison_result,
        result_version=(
            candidate_binding.indeterminate_byte_length_comparison_result.result_version + 1
        ),
    )
    ...
    changed_recorded = replace(
        recorded,
        indeterminate_byte_length_comparison_result_binding=changed_candidate_binding,
    )
