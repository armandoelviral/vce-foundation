...
def create_basis() -> SecurityAdmissionCandidateMediaTypeComparisonBasis:
    ...
    declaration = SecurityAdmissionCandidateDeclaredMediaType(
        ...
        declared_media_type="future/image-format",
    )
    ...
    detection = SecurityAdmissionCandidateDetectedMediaTypeObservation(
        observation_id="observation-001",
        observation_version=1,
        candidate_identity=create_candidate(),
        verification_procedure_identity=procedure,
        detected_media_type="application/x-future-image",
...
def create_result() -> (
    ...
    return SecurityAdmissionCandidateIndeterminateMediaTypeComparisonResult(
...
def test_indeterminacy_reason_vocabulary_is_exact_and_closed() -> None:
    assert tuple(SecurityAdmissionMediaTypeComparisonIndeterminacyReason) == (
        SecurityAdmissionMediaTypeComparisonIndeterminacyReason
    ...
    assert tuple(
        member.value
        for member in SecurityAdmissionMediaTypeComparisonIndeterminacyReason
    ) == (
        "UNSUPPORTED_COMPARISON_SCHEME",
        "NONCOMPARABLE_MEDIA_TYPE_EVIDENCE",
    )
...
@pytest.mark.parametrize(
    "reason",
    list(SecurityAdmissionMediaTypeComparisonIndeterminacyReason),
)
def test_both_indeterminacy_reasons_are_accepted(
...
def test_complete_noncomparable_evidence_remains_recoverable() -> None:
    result = create_result()
...
def test_insufficient_evidence_reason_is_absent() -> None:
    values = {
        member.value
        for member in SecurityAdmissionMediaTypeComparisonIndeterminacyReason
    }
