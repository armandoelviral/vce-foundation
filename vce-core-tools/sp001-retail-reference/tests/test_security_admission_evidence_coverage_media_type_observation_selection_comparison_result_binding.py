...
from tests.test_security_admission_evaluation_record_media_type_observation_selection_comparison_result_binding import (
    create_binding as create_governed_recorded_binding,
)
from tests.test_security_admission_evidence_coverage_media_type_comparison_result_binding import (
...
def create_binding(
    ...
    coverage_binding = (
        SecurityAdmissionEvidenceCoverageMediaTypeComparisonResultBinding(
            ...
            evaluation_record_media_type_comparison_result_binding=recorded,
        )
    )
    return (
        SecurityAdmissionEvidenceCoverageMediaTypeObservationSelectionComparisonResultBinding(
...
def test_fields_are_exact() -> None:
    ...
    assert (
        ...
        is SecurityAdmissionEvaluationRecordMediaTypeObservationSelectionComparisonResultBinding
    )
    assert (
        binding_fields[1].type
        is SecurityAdmissionEvidenceCoverageMediaTypeComparisonResultBinding
    )
...
def test_binding_is_immutable_and_slotted() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        binding.evidence_coverage_media_type_comparison_result_binding = (  # type: ignore[misc]
...
def test_exact_references_are_preserved() -> None:
    ...
    coverage = (
        binding.evidence_coverage_media_type_comparison_result_binding
    )
    reconstructed = (
        SecurityAdmissionEvidenceCoverageMediaTypeObservationSelectionComparisonResultBinding(
            evaluation_record_media_type_observation_selection_comparison_result_binding=(
            ...
            evidence_coverage_media_type_comparison_result_binding=coverage,
        )
    )
    assert (
        reconstructed
...
def test_equal_reconstruction_has_value_equality() -> None:
    ...
    reconstructed = replace(
        binding,
        evaluation_record_media_type_observation_selection_comparison_result_binding=replace(
            binding
            .evaluation_record_media_type_observation_selection_comparison_result_binding
        ),
        evidence_coverage_media_type_comparison_result_binding=replace(
            binding.evidence_coverage_media_type_comparison_result_binding
        ),
    )
...
def test_different_recorded_result_binding_is_rejected() -> None:
    ...
    recorded = (
        coverage.evaluation_record_media_type_comparison_result_binding
    )
    candidate_binding = recorded.media_type_comparison_result_binding
    changed_result = replace(
        candidate_binding.media_type_comparison_result,
        result_version=(
            candidate_binding.media_type_comparison_result.result_version + 1
        ),
    )
    changed_candidate_binding = replace(
        candidate_binding,
        media_type_comparison_result=changed_result,
    )
