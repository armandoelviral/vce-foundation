...
class RetailProductObservedCost:
    """Immutable exact product cost observed in one retail context."""

    product_identity: RetailProductIdentity
    context_scope: RetailContextScope
    monetary_amount: RetailCommercialMonetaryAmount
    costing_scheme_id: str
    costing_scheme_version: int
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.costing_scheme_id, str):
            raise TypeError("costing_scheme_id must be a string")

        if not self.costing_scheme_id:
            raise ValueError("costing_scheme_id must not be empty")

        if (
            isinstance(self.costing_scheme_version, bool)
            or not isinstance(self.costing_scheme_version, int)
        ):
            raise TypeError(
                "costing_scheme_version must be an integer"
            )

        if self.costing_scheme_version <= 0:
            raise ValueError(
                "costing_scheme_version must be positive"
            )
